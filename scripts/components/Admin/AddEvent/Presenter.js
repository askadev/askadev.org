...
export default class extends React.Component {
  ...
  render() {
    return (
      <FormLayout
        title={`Create a new Event in ${this.props.regionDisplayName}`}
      >
        <form onSubmit={this.handleSubmit}>
          ...
          <DateInput label="Event Date" onChange={val => (this.date = val)} />
          <TimeInput
            label="Start Time"
            onChange={val => (this.startTime = val)}
          />
          <TimeInput label="End Time" onChange={val => (this.endTime = val)} />
